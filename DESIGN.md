...
- **Agents**: data stream consumers that may perform actions such as placing bets upon receiving update from
    ...
    a background task for consuming data streams. 
- **Operator**: act as the broker for all the bets placed by the agents as well as keeping track of each
    agent's account balance. For simplicity of first design, we implement this as a module exposing a tool interface
    for agents, rather than an independent service.
