...
async def demo_websearch_integration():
    ...
    try:
        result = await api_tavily.fetch("search", {"query": "Lakers vs Celtics injury odds", "max_results": 5})
    ...
    print("-" * 70)
    try:
        result = await api_tavily.fetch("search", {"query": "NBA injury updates", "max_results": 3})
        print(f"  Results: {result['total_results']}")
    ...
    print("-" * 70)
    try:
        result = await api_tavily.fetch("search", {
