...
class DataProcessor(ABC):
    ...
    def should_process(self, event: DataEvent) -> bool:
        ...
        if event_intent is not None and processor_intent is not None:
            ...
            event_intent_str = (
                event_intent.value
                if isinstance(event_intent, Enum)
                else str(event_intent)
            )
            processor_intent_str = (
                processor_intent.value
                if isinstance(processor_intent, Enum)
                else str(processor_intent)
            )
    ...
    async def process_batched(
        ...
        results = await asyncio.gather(
            *[self.process(event) for event in events], return_exceptions=True
        )
