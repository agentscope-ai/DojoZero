    ...
    async def load_state(
        ...
        Args:
            ...
            dedup_keys: Optional set of deduplication keys extracted from JSONL.
                       ...
                       - "{game_id}_drive_{drive_id}" for NFL drives
        ...
        if dedup_keys is not None:
            # NFL drive events: {game_id}_drive_{drive_id}
            drive_ids = {k for k in dedup_keys if "_drive_" in k}
            if drive_ids:
                self._state.rebuild_dedup_from_drive_ids(drive_ids)
