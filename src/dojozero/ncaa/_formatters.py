"""Event formatters for NCAA betting agent.

NCAA basketball uses the same game structure as NBA (halves instead of
quarters in college, but ESPN normalizes this to periods). Formatters
mirror NBA but reference NCAA event classes.
"""
...
def _format_game_update(event: NCAAGameUpdateEvent) -> str:
    period = event.period
    game_clock = event.game_clock
    home = event.home_team_stats
    away = event.away_team_stats

    # NCAA uses 2 halves; ESPN reports as period 1, 2, then OT
    period_name = f"H{period}" if period <= 2 else f"OT{period - 2}"
...
def _format_play_by_play(event: NCAAPlayEvent) -> str:
    period = event.period
    clock = event.clock
...
_EVENT_FORMATTERS: dict[str, Any] = {
    ...
    **SOCIALMEDIA_EVENT_FORMATTERS,
    ...
    "ncaa_game_update": _format_game_update,
    "odds_update": _format_odds_update,
    "ncaa_play": _format_play_by_play,
