...
class NFLStreamInitializer:
    """Stream initializer that triggers NFL pre-game web searches.
    ...
    def __init__(
        ...
        Args:
            ...
            home_team_name: Home team full name (e.g., "Kansas City Chiefs")
            away_team_name: Away team full name (e.g., "San Francisco 49ers")
            game_date: Game date string (e.g., "2025-01-15")
            home_team_abbreviation: Home team abbreviation (e.g., "KC")
            away_team_abbreviation: Away team abbreviation (e.g., "SF")
        ...
        self._home_team_abbreviation = home_team_abbreviation
        self._away_team_abbreviation = away_team_abbreviation
        self._week = week
    ...
    def _render_template(self, template: str) -> str:
        ...
        if "home_abbrev" in placeholders:
            replacements["home_abbrev"] = self._home_team_abbreviation or ""

        # {away_abbrev} - Away team abbreviation
        if "away_abbrev" in placeholders:
            replacements["away_abbrev"] = self._away_team_abbreviation or ""

        # {week} - NFL week number
        if "week" in placeholders:
            replacements["week"] = str(self._week) if self._week else ""
    ...
    def _generate_default_queries(self) -> list[tuple[str, WebSearchIntent]]:
        ...
        week_str = ""
        if self._week:
            week_str = f" Week {self._week}"

        queries = []

        # Injury report query
        if teams_str:
            injury_query = f"NFL injury updates for {teams_str}{date_str}"
            queries.append((injury_query, WebSearchIntent.INJURY_SUMMARY))
        else:
            queries.append(("NFL injury updates", WebSearchIntent.INJURY_SUMMARY))

        # Power ranking query
        queries.append((f"NFL power rankings{week_str}", WebSearchIntent.POWER_RANKING))

        # Expert prediction query
        if teams_str:
            prediction_query = f"NFL expert predictions for {teams_str}{date_str}"
            queries.append((prediction_query, WebSearchIntent.EXPERT_PREDICTION))
        else:
            queries.append(
                ("NFL expert predictions", WebSearchIntent.EXPERT_PREDICTION)
            )
