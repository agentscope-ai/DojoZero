...
Orchestrates betting trials for NCAA basketball (March Madness) games:
- Checks ESPN API for daily NCAA games
...
Usage:
    ...
    python ncaa_trial_runner.py run --data-dir outputs

    # Server mode (with SLS/OSS via Dashboard Server)
    # First start: dojo0 serve --otlp-endpoint https://... --trace-backend sls
    python ncaa_trial_runner.py run --data-dir outputs --server http://localhost:8000
"""
...
from dojozero.data.ncaa._utils import get_game_info_by_id, get_games_for_date

logger = logging.getLogger(__name__)


class GameTrialManager:
    """Manages trial lifecycle for a single NCAA game."""
    ...
    def generate_config_file(self) -> Path:
        ...
        if self.data_dir:
            ...
            config_file = configs_dir / f"ncaa-moneyline_{self.game_id}.yaml"
            events_file = outputs_dir / f"ncaa_betting_events_{self.game_id}.jsonl"
    ...
    async def _get_current_game_status(self) -> int | None:
        """Get current game status from ESPN API.
...
def list_games_in_range(
    ...
    print(f"\n{'=' * 80}")
    print(f"NCAA Games from {start.strftime('%Y-%m-%d')} to {end.strftime('%Y-%m-%d')}")
    print(f"{'=' * 80}\n")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="NCAA Trial Runner - Orchestrates betting trials for NCAA basketball games"
    )
    subparsers = parser.add_subparsers(dest="command", help="Available commands")

    # List games subcommand
    list_parser = subparsers.add_parser("list", help="List NCAA games in a date range")
    ...
    run_parser = subparsers.add_parser("run", help="Run betting trials for NCAA games")
    ...
    run_parser.add_argument(
        "--config",
        type=Path,
        default=Path(__file__).parent.parent / "trial_params" / "ncaa-moneyline.yaml",
        help="Path to trial config template (default: trial_params/ncaa-moneyline.yaml)",
    )
    run_parser.add_argument(
        "--data-dir",
        type=Path,
